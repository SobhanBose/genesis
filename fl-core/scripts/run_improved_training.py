...
class AdvancedTrainer:
    ...
    def create_model(self, input_size: int) -> nn.Module:
        ...
        if model_type == 'improved':
            ...
            model = create_model(
                ...
                ensemble_member_type=ensemble_config.get('ensemble_model_type', 'improved')
            )
    ...
    def get_scheduler(self, optimizer: optim.Optimizer) -> optim.lr_scheduler._LRScheduler:
        ...
        if scheduler_type == 'cosine_annealing':
            scheduler = optim.lr_scheduler.CosineAnnealingLR(
                optimizer, 
                T_max=training_config.get('cosine_t_max', 50)
            )
    ...
    def get_loss_function(self) -> nn.Module:
        ...
        if loss_type == 'focal':
            loss_kwargs.update({
                'alpha': training_config.get('focal_alpha', 1.0),
            ...
            loss_kwargs.update({
                'uncertainty_weight': training_config.get('uncertainty_weight', 0.1)
            })
    ...
    def preprocess_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if feature_config.get('feature_selection_enabled', False):
            if feature_config.get('feature_selection_method'):
                ...
                X_selected = self.feature_engineer.select_features(
                    ...
                    k=feature_config.get('n_features', 100)
                )
        ...
        if feature_config.get('use_pca', False):
            X = df.drop(['class'], axis=1)
            y = df['class'].astype(int)  # Ensure y is a Series with proper type
            X_reduced = self.feature_engineer.apply_pca(
                X, n_components=feature_config.get('pca_components', 50)
            )
        ...
        if feature_config.get('scaling_method'):
            ...
            X_scaled = self.feature_engineer.scale_features(
                X, method=feature_config['scaling_method']
            )
    ...
    def train_epoch(self, model: nn.Module, train_loader: DataLoader, 
                   optimizer: optim.Optimizer, loss_fn: nn.Module) -> Tuple[float, float, float, float, float, float]:
    ...
    def evaluate(self, model: nn.Module, test_loader: DataLoader, 
                loss_fn: nn.Module) -> Tuple[float, float, float, float, float, float]:
    ...
    def train_with_kfold_cv(self, train_data: pd.DataFrame, val_data: pd.DataFrame) -> Dict:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X)):
            ...
            train_dataset = ImprovedClinVarDataset(train_fold_data)
            test_dataset = ImprovedClinVarDataset(test_fold_data)
            ...
            if use_weighted_sampler:
                ...
                weights = 1. / np.array(class_sample_count, dtype=np.float32)
            ...
            for epoch in range(max_epochs):
                # Train
                train_loss, train_acc, train_roc_auc, train_prec, train_rec, train_f1 = self.train_epoch(model, train_loader, optimizer, loss_fn)
                # Per-epoch evaluation on test set
                test_loss, test_acc, test_roc_auc, test_prec, test_rec, test_f1 = self.evaluate(model, test_loader, loss_fn)
                ...
                if (epoch + 1) % 10 == 0:
                    self.logger.log_info(
                        ...
                        f"Train Loss: {train_loss:.4f}, Acc: {train_acc:.4f}, ROC_AUC: {train_roc_auc:.4f}; "
                        f"Test Loss: {test_loss:.4f}, Acc: {test_acc:.4f}, ROC_AUC: {test_roc_auc:.4f}"
                    )
            ...
            final_test_loss, final_test_acc, final_test_roc_auc, final_test_prec, final_test_rec, final_test_f1 = self.evaluate(model, test_loader, loss_fn)
            ...
            self.logger.log_info(f"✅ Fold {fold_idx + 1} completed - Test Acc: {final_test_acc:.4f}, ROC_AUC: {final_test_roc_auc:.4f}, F1: {final_test_f1:.4f}")
        ...
        model = self.create_model(len(val_dataset.feature_cols))
        model.load_state_dict(torch.load(best_model_path))
        loss_fn = self.get_loss_function()
        
        final_val_loss, final_val_acc, final_val_roc_auc, final_val_prec, final_val_rec, final_val_f1 = self.evaluate(model, val_loader, loss_fn)
        ...
        self.logger.log_info(f"🎯 Final Validation Results - Loss: {final_val_loss:.4f}, Acc: {final_val_acc:.4f}, ROC_AUC: {final_val_roc_auc:.4f}, F1: {final_val_f1:.4f}")
    ...
    def train(self, train_data: pd.DataFrame, test_data: pd.DataFrame, val_data: pd.DataFrame) -> Dict:
        ...
        if use_cv:
            ...
            final_train_loss, final_train_acc, final_train_roc_auc, final_train_prec, final_train_rec, final_train_f1 = self.evaluate(model, train_loader, loss_fn)
            final_test_loss, final_test_acc, final_test_roc_auc, final_test_prec, final_test_rec, final_test_f1 = self.evaluate(model, test_loader, loss_fn)
            final_val_loss, final_val_acc, final_val_roc_auc, final_val_prec, final_val_rec, final_val_f1 = self.evaluate(model, val_loader, loss_fn)
