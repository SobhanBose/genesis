...
class GenomicFeatureEngineer:
    ...
    def scale_features(self, X: pd.DataFrame, method='robust') -> pd.DataFrame:
        """
    ...
    def engineer_all_features(self, df: pd.DataFrame, target_col='class') -> pd.DataFrame:
        ...
        - Label encode 'chr'
        - Remove 'gene'
        - Embed 'ref' and 'alt' with Word2Vec, drop originals
        - Keep all other columns (except 'gene', 'ref', 'alt') and the target 'class'
        - Fill NaN with 0
        """
        print("🔧 Feature engineering pipeline (keep all except gene, ref, alt)...")
        df = df.copy()
        ...
        # Remove 'gene' column if present
        if 'gene' in df.columns:
            df = df.drop(columns=['gene'])

        # Embed 'ref' and 'alt' using Word2Vec, then drop originals
    ...
    def embed_ref_alt_sequences(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Create embeddings for ref and alt sequences using pre-trained Word2Vec models
        """
        df = df.copy()
        if not self.word2vec_models:
            print("⚠️ Word2Vec models not loaded. Loading default models...")
            self.load_word2vec_models()
        if not self.kmer_generator:
            self.create_kmer_generator()
        def embed_sequence(seq, model, k=3):
            if seq is None or pd.isna(seq):
                return np.zeros(model.vector_size)
            tokens = self.kmer_generator.generate_kmers(str(seq).upper())
            ...
            if vectors:
                ...
                return np.zeros(model.vector_size)
        # Embed ref sequences
        if 'ref' in self.word2vec_models and 'ref' in df.columns:
            ref_embeddings = df['ref'].apply(lambda x: embed_sequence(x, self.word2vec_models['ref']))
            ref_emb_df = pd.DataFrame(ref_embeddings.tolist(), columns=[f'ref_emb_{i}' for i in range(self.word2vec_models['ref'].vector_size)], index=df.index)
            df = pd.concat([df, ref_emb_df], axis=1)
        # Embed alt sequences
        if 'alt' in self.word2vec_models and 'alt' in df.columns:
            alt_embeddings = df['alt'].apply(lambda x: embed_sequence(x, self.word2vec_models['alt']))
            alt_emb_df = pd.DataFrame(alt_embeddings.tolist(), columns=[f'alt_emb_{i}' for i in range(self.word2vec_models['alt'].vector_size)], index=df.index)
...
class AdvancedEmbeddingGenerator:
    ...
    def create_multi_scale_embeddings(self, ref_sequences: List[str], alt_sequences: List[str]) -> Dict:
        ...
        for k in self.k_mer_sizes:
            ...
            # Embed all sequences
            ref_embedded = np.array([self.embed_sequence(seq, ref_embeddings, ref_kmer_to_idx, k) 
                                   for seq in ref_sequences])
            alt_embedded = np.array([self.embed_sequence(seq, alt_embeddings, alt_kmer_to_idx, k) 
