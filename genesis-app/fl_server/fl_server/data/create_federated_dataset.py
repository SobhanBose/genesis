...
class FederatedDatasetManager:
    ...
    def create_iid_partition(self, n_clients: int, seed: int = 42) -> Dict[int, Dict[str, pd.DataFrame]]:
        ...
        for client_id in range(n_clients):
            ...
            train_data, test_data = train_test_split(client_df, test_size=0.2, random_state=seed, stratify=client_df.get("label"))
    ...
    def create_non_iid_partition(self, n_clients: int, alpha: float = 0.5, seed: int = 42) -> Dict[int, Dict[str, pd.DataFrame]]:
        ...
        for client_id in range(n_clients):
            if client_data[client_id]["train"]:
                ...
                if len(client_df) > 1:
                    try:
                        train_data, test_data = train_test_split(client_df, test_size=0.2, random_state=seed, stratify=client_df["class"])
                ...
                final_client_data[client_id] = {"train": train_data, "test": test_data}
    ...
    def create_pytorch_datasets(self, client_data: Dict[int, Dict[str, pd.DataFrame]], feature_cols: List[str]) -> Dict[int, Dict[str, PathogenicityDataset]]:
        """Convert pandas DataFrames to PyTorch Datasets"""
        pytorch_datasets = {}

        for client_id, data in client_data.items():
            pytorch_datasets[client_id] = {"train": PathogenicityDataset(data["train"], feature_cols), "test": PathogenicityDataset(data["test"], feature_cols)}
    ...
    def get_data_statistics(self, client_data: Dict[int, Dict[str, pd.DataFrame]]) -> Dict:
        ...
        for client_id, data in client_data.items():
            ...
            stats["clients"][client_id] = {"train_samples": len(data["train"]), "test_samples": len(data["test"]), "train_label_dist": train_labels, "test_label_dist": test_labels}
...
def create_federated_datasets(data_dir: str, output_dir: str, n_clients: int, partition_type: str = "non_iid", alpha: float = 0.5, feature_cols: Optional[List[str]] = None, seed: int = 42):
    """
    ...
    return {"client_data": client_data, "pytorch_datasets": pytorch_datasets, "validation_dataset": validation_dataset, "statistics": stats, "partition_dir": partition_dir}
